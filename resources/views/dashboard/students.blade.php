...
<div style="padding:28px 32px;height:100%;overflow:auto" class="scroll"
     x-data="studentsPage()" x-init="init()">
        ...
        <tr style="border-bottom:0.5px solid var(--line);transition:background .14s"
          ...
          <td style="padding:12px">
            <div class="row-actions" style="display:flex;gap:5px;opacity:0;transition:opacity .15s">
              <button @click="openEdit({{ $s->id }}, '{{ addslashes($s->name) }}', '{{ addslashes($s->email) }}')"
              ...
              @if($s->is_active)
              <form action="{{ route('dashboard.students.suspend', $s) }}" method="POST"
                    onsubmit="return confirm('Suspendre {{ addslashes($s->name) }} ?')">
                @csrf
              ...
              @else
              <form action="{{ route('dashboard.students.unsuspend', $s) }}" method="POST">
                @csrf
                <button type="submit" style="height:28px;padding:0 10px;border-radius:7px;border:0.5px solid var(--line-2);background:var(--surface);font:500 11px/1 var(--f-ui);color:var(--ink-2);cursor:pointer">
                  Rétablir
                </button>
              </form>
              @endif
    ...
    <div style="position:relative;width:100%;max-width:520px;background:var(--surface);border-radius:18px;border:0.5px solid var(--line);box-shadow:0 32px 80px -20px rgba(20,21,43,.25);overflow:hidden">
      ...
      <div style="display:flex;align-items:center;gap:12px;padding:22px 24px;border-bottom:0.5px solid var(--line)">
        <div style="width:36px;height:36px;border-radius:10px;background:var(--ink);display:flex;align-items:center;justify-content:center;flex-shrink:0">
          <x-ui.icon name="classes" size="16" style="color:var(--bg)"/>
        </div>
        <div>
          <h3 style="font:400 20px/1 var(--f-display);margin:0" x-text="modal.editing ? 'Modifier l\'élève' : 'Nouvel élève'"></h3>
          <p style="font:400 11.5px/1 var(--f-ui);color:var(--ink-3);margin:4px 0 0" x-text="modal.editing ? 'Mettre à jour les informations' : 'Créer un compte élève'"></p>
        </div>
      ...
      <div style="padding:22px 24px 24px">
        <template x-if="!modal.editing">
          <form action="{{ route('dashboard.students.store') }}" method="POST" style="display:flex;flex-direction:column;gap:14px">
            @csrf
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
              <div style="grid-column:1/-1">
                ...
                <input name="name" required placeholder="Sami Ouahbi"
              ...
              <div>
                ...
                <input name="email" type="email" required placeholder="sami@majakker.ma"
                       style="width:100%;height:40px;padding:0 12px;border-radius:9px;border:0.5px solid var(--line-2);background:var(--surface-2);font:400 13.5px/1 var(--f-ui);color:var(--ink);outline:none;box-sizing:border-box"
              ...
              <div>
                ...
                <div style="position:relative">
                  <input name="password" type="password" id="modal-pwd-st" required
                  ...
                  <button type="button"
                          onclick="var i=document.getElementById('modal-pwd-st');i.type=i.type==='password'?'text':'password'"
        ...
        <template x-if="modal.editing">
          <form :action="'/dashboard/students/'+modal.userId" method="POST" style="display:flex;flex-direction:column;gap:14px">
            @csrf
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
              ...
              <div>
                ...
                <div style="position:relative">
                  <input name="password" type="password" id="modal-pwd-se"
                         placeholder="Laisser vide pour conserver"
                         style="width:100%;height:40px;padding:0 36px 0 12px;border-radius:9px;border:0.5px solid var(--line-2);background:var(--surface-2);font:400 13.5px/1 var(--f-ui);color:var(--ink);outline:none;box-sizing:border-box"/>
                  <button type="button"
                          onclick="var i=document.getElementById('modal-pwd-se');i.type=i.type==='password'?'text':'password'"
                          style="position:absolute;right:10px;top:50%;transform:translateY(-50%);border:0;background:0;cursor:pointer;color:var(--ink-3);display:flex;align-items:center;padding:4px">
                    <svg width="14" height="14" viewBox="0 0 20 20" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round">
                      <path d="M2 10 C4 6 7 4 10 4 C13 4 16 6 18 10 C16 14 13 16 10 16 C7 16 4 14 2 10Z"/>
                      <circle cx="10" cy="10" r="2.5"/>
...
function studentsPage() {
  return {
    ...
    bindFields() {
      const f = document.querySelector('[x-data="studentsPage()"] form');
      if (!f) return;
